...
const createForkedSession = `-- name: CreateForkedSession :one
INSERT INTO sessions (
    ...
    strftime('%s', 'now'),
    strftime('%s', 'now')
) RETURNING id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, forked_from_session_id
`

type CreateForkedSessionParams struct {
	ID                  string         `json:"id"`
...
func (q *Queries) CreateForkedSession(ctx context.Context, arg CreateForkedSessionParams) (Session, error) {
	row := q.queryRow(ctx, q.createForkedSessionStmt, createForkedSession,
		arg.ID,
		arg.Title,
		arg.MessageCount,
		arg.PromptTokens,
...
SELECT id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, forked_from_session_id
FROM sessions
...
const listForks = `-- name: ListForks :many
SELECT id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, forked_from_session_id
FROM sessions
...
func (q *Queries) ListForks(ctx context.Context, forkedFromSessionID sql.NullString) ([]Session, error) {
	rows, err := q.query(ctx, q.listForksStmt, listForks, forkedFromSessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
