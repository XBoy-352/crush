...
const copyMessagesUpToCheckpoint = `-- name: CopyMessagesUpToCheckpoint :exec
...
WHERE m.session_id = ?2
  AND m.rowid < (
      ...
      WHERE cp.id = ?3 AND cp.session_id = ?2
  )
ORDER BY m.rowid ASC
`

type CopyMessagesUpToCheckpointParams struct {
...
func (q *Queries) CopyMessagesUpToCheckpoint(ctx context.Context, arg CopyMessagesUpToCheckpointParams) error {
	_, err := q.exec(ctx, q.copyMessagesUpToCheckpointStmt, copyMessagesUpToCheckpoint, arg.NewSessionID, arg.SessionID, arg.CheckpointID)
	return err
}
