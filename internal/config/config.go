...
type Options struct {
	...
	// WebSearchEngine selects the backend for the web_search tool used by
	// agentic_fetch. Empty or "duckduckgo" queries DuckDuckGo; "searxng"
	// queries the SearXNG instance at SearxngURL and falls back to
	// DuckDuckGo when the instance is unreachable.
	WebSearchEngine string `json:"web_search_engine,omitempty" jsonschema:"description=Web search backend for the web_search tool used by agentic_fetch. SearXNG falls back to DuckDuckGo when the instance is unreachable,enum=duckduckgo,enum=searxng,default=duckduckgo"`
	// SearxngURL is the base URL of the SearXNG instance used when
	// WebSearchEngine is searxng. The instance must enable the json
	// format in its settings.
	SearxngURL    string                `json:"searxng_url,omitempty" jsonschema:"description=Base URL of the SearXNG instance used when web_search_engine is searxng. The instance must enable the json format in its settings,default=http://localhost:8888,example=http://localhost:8888"`
	RemoteControl *RemoteControlOptions `json:"remote_control,omitempty" jsonschema:"description=Session-attached remote control relay settings (password via CRUSH_REMOTE_PASS)"`
}

// RemoteControlOptions configures the session-attached remote control
// bridge. Password is never stored here; use CRUSH_REMOTE_PASS.
